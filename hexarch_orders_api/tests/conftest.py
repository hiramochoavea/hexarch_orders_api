...
def reverse_url() -> Callable[[str, tuple, dict], str]:
    ...
    def _reverse_url(name: str, *args: tuple, **kwargs: dict) -> str:
...
def initial_item(db) -> ItemModel:
    ...
    return ItemModel.objects.create(
        ...
        description="The latest Samsung Galaxy S23 with a 6.1-inch display, "
        "128GB storage, and 5G support.",
...
def initial_order(db, initial_item: ItemModel) -> Order:
    ...
    OrderItemModel.objects.create(order=order, item=initial_item, quantity=quantity)
